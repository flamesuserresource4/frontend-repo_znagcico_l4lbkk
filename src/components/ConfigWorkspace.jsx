  ...
  return (
    <div className="p-4 space-y-4">
      <motion.div
        ...
        className="rounded-2xl bg-white/60 backdrop-blur-xl ring-1 ring-white/40 shadow-lg p-4"
      >
        <div className="flex items-center justify-between">
          ...
          {!hasNumber && (
            <motion.button
              ...
              className="px-4 py-2 rounded-xl bg-gradient-to-r from-[#8A5CF6] to-[#6C63FF] text-white text-sm font-medium shadow-lg"
            >
              Add Number
            </motion.button>
          )}
      ...
      <AnimatePresence mode="wait">
        {hasNumber ? (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -8 }}
            className="rounded-2xl bg-white/60 backdrop-blur-xl ring-1 ring-white/40 shadow-lg"
          >
          ...
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -8 }}
            className="relative overflow-hidden rounded-2xl bg-white/60 backdrop-blur-xl ring-1 ring-white/40 shadow-lg p-8 text-center text-gray-700"
          >
            <div className="pointer-events-none absolute -inset-20 bg-gradient-to-br from-[#8A5CF6]/10 via-[#6C63FF]/5 to-transparent blur-3xl" />
            <div className="relative z-0 inline-flex items-center gap-2 rounded-xl bg-white/70 ring-1 ring-white/50 px-3 py-1 text-xs text-gray-700">
              <Sparkles className="h-3.5 w-3.5 text-[#8A5CF6]" /> Minimal Flow Builder Canvas
            </div>
            <div className="mt-3">Connect a number to get started with {type.toLowerCase()} flows.</div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
